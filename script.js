...
async function fetchWords() {
    try {
        ...
        return data.filter(word => word.length <= 6); // Filter for simpler words
    } catch (error) {
...
function checkInput() {
    ...
    if (inputText === textDisplay.textContent) {
        ...
        textInput.value = '';
        setNewWord();
    } else if (inputText && inputText !== textDisplay.textContent.substring(0, inputText.length)) {
        errors++;
...
function saveResult() {
    if (!started) {
        alert('You need to start the test first.');
        return;
    }
    const wpm = parseInt(wpmElement.textContent);
